...
namespace io {
	static Button   buttons[MAX_NUM_KEYS];
	static std::map<int, int> action_to_key;
	...
	void map_action_to_button(const int action, const Button & button)
	{
		...
		action_to_key[action] = button.m_bound_key; // map the action to the button
		buttons[button.m_bound_key] = button;
	...
	const Button & get_button_from_action(const int action) 
	{
		std::map<int, int>::const_iterator iter = action_to_key.find(action);
		if (iter != action_to_key.cend()) {
			return buttons[iter->second];
		}
#ifndef NDEBUG
