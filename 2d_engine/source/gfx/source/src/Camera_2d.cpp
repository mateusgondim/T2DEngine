	...
	m_tile_width(tile_width), m_tile_height(tile_height), m_screen_tile_width(screen_tile_width), m_screen_tile_height(screen_tile_height) 
{
	...
	m_projection = math::ortho(-floor(width / (float)2), ceil(width / (float)2), -floor(height / (float)2), ceil(height  / (float)2), -1.0, 1.0f);
	m_transform.get_translation() = math::vec3(floor(width / (float)2), floor(height / (float)2), 0.0f);
