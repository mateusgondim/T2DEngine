...
Gameplay_state * Player_climbing_state::handle_input(Actor & actor)
{
	...
	bool is_on_ladder = g_engine.m_physics_manager.get_world()->is_body_on_ladder(actor.get_body_2d());
	bool is_on_ladder_top = g_engine.m_physics_manager.get_world()->is_on_ladder_top_tile(actor.get_body_2d(), bounds);
	bool is_on_ground = g_engine.m_physics_manager.get_world()->is_body_2d_on_ground(actor.get_body_2d());
	//climbing from top animation and small displacement
	if (m_from_top) {
		if (m_anim_clip == 0) {
			actor.get_sprite().get_panim_controller()->switch_curr_state_anim_clip(1);
			actor.get_body_2d()->set_velocity(-m_climbing_vel);
		}
		if (bounds.y > actor.get_body_2d()->get_aabb_2d().p_max.y) {
			actor.get_body_2d()->stop_movement_y();
			actor.get_sprite().get_panim_controller()->switch_curr_state_anim_clip(0);
			m_from_top = false;
		}
		return nullptr;
	}
		...
		if (!m_reached_top) { //in the last frame it was still climbing the ladder 
			...
			if (!is_on_ladder) { // have fallen of
				...
				actor.get_sprite().get_panim_controller()->set_bool("is_climbing", false);
			...
			if (moving_up_button.m_state == PRESSED) {
				actor.get_body_2d()->set_velocity(m_climbing_vel);
				actor.get_sprite().get_panim_controller()->get_current_state().get_anim_player().resume();
			}
			else {
				...
				if (moving_down_button.m_state == PRESSED) {
					...
					actor.get_body_2d()->stop_movement_y();
					actor.get_sprite().get_panim_controller()->get_current_state().get_anim_player().pause();
				}
			}

			if (is_on_ladder_top) {//REACHED THE TOP
				//std::cout << "-----------------reached top of ladder---------- " << std::endl;
				if (actor.get_body_2d()->get_velocity().y > 0.0f && (bounds.y <= actor.get_body_2d()->get_aabb_2d().p_max.y)) {
					m_reached_top = true;
					actor.get_body_2d()->set_velocity(cgm::vec2(0.0f, m_climbing_vel.y));
			...
			float center_y = (actor.get_body_2d()->get_aabb_2d().p_max.y + actor.get_body_2d()->get_aabb_2d().p_min.y) / 2.0f;
